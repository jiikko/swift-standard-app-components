...
UI 非依存のロギング product。**ログの入口を `AppLog` に統一**し (callsite は `Logger(subsystem:category:)` を手打ちしない)、用途で 2 レーンを使い分ける。

| レーン | 入口 | privacy | sink |
|---|---|---|---|
| **line log** | `appLog.info(.app, "msg")` 等 | category 単位 / メッセージ全体 | `os.Logger` 常時 + DEBUG stderr ミラー |
| **structured / OSLog-native** | `appLog.osLogger(for: .network)` → `Logger` | 補間ごとに `privacy:` 明示 | `os.Logger` のみ |
...
- **補間ごとに privacy を分けたいときは structured レーン** (`appLog.osLogger(for:)`) を使い、返った `Logger` に直接書く。`os.Logger` のメソッドは補間リテラルを呼び出し箇所に要求するため、薄い wrapper (`structured().error(OSLogMessage)`) は原理的に作れず raw `Logger` を返す形になっている (詳細は [docs/logging.md](docs/logging.md))。
