...
採用判断のチェックリスト。各 API 表の前に置く一段階上のフィルタ。

| API | 採用すべきアプリ種別 | 採用しない |
|-----|---------------------|-----------|
| `SettingsWindow` / `GeneralTabContract` | macOS で `Settings` シーンを持つ全アプリ | コマンド非インタラクティブ (LSUIElement = NO の "settings" 概念がないアプリ等) |
| `View.applyAppAppearance(_:)` | アプリ全体の外観 (light/dark/system) をユーザーに選ばせるアプリ | 外観 = OS 追従固定で問題ないアプリ (= ほとんどのアプリ。明示的に「Light/Dark を切り替えたい」要件がある時だけ) |
| `View.standardSettingsBehaviors()` / `WindowBackgroundView` / `View.autoSaveWindowFrame(_:)` | macOS 標準ウィンドウ挙動を踏襲したいアプリ全般 | iPad のような plain 単一フルスクリーン UI (= macOS 専用 lib なので無関係) |
| `LaunchAtLoginToggle` / `LaunchAtLoginService` | **メニューバー常駐ユーティリティ** (clipboard / window manager / 同期クライアント) | **ドキュメント編集アプリ / メディアプレイヤー** ── ユーザーが「使いたい時だけ開く」UX が前提のアプリは login 時自動起動を入れない (Apple の Pages / Keynote / Final Cut にも無い) |
| `MenuBarVisibilityToggle` | メインウィンドウ + オプショナルなメニューバー常駐の **両方** を持つアプリ | メニューバー常駐 only のアプリ (= 常時表示固定のため toggle 不要)、メニューバー機能を持たないアプリ |
| `StandardAppComponentsLocalization.validateRequiredKeys()` | lib の Settings 周り API を採用した **全アプリ** (起動時に 1 度呼ぶ。`fatalError` するため設計通り) | lib の API を 1 つも採用していないアプリ (検証対象がない) |

判断に迷ったら 「**この機能を有効化することで普通のユーザーが本当に得をするか**」 を考える。Launch at Login は menu bar utility では「常時起動」が機能の前提だが、document editor では Dock を汚すだけになる。
...
| `LanguageSection` | `GeneralTabContract.language` slot にそのまま流し込む言語 Picker。`supportedLanguages: [LanguageOption]` で対応言語を指定、`onRestart: () -> Void` で「いますぐ再起動」押下時の挙動を差し替える (デフォルトは `NSApp.terminate(nil)`)。Picker 変更で即時 apply → `Later / Restart Now` の alert を出す UX (macOS System Settings に揃える)。`AppleLanguages` UserDefaults 操作は lib 側で完結。 |
...
| `MenuBarVisibilityToggle` | 「メニューバーに表示」を切り替える labeled Toggle。`Binding<Bool>` を受け取るのみで、**NSStatusItem の生成 / 破棄やアイコン / メニュー実装は consumer 側に閉じる** (各アプリ固有のため)。consumer は binding の変化を観測して NSStatusItem を生成 / 破棄する。**境界**: lib に存在する付加価値は「localized label "Show in Menu Bar" を複数アプリで共有する」点だけ。同種の薄い Toggle ラッパー (`AutoUpdatesToggle` 等) を増やす場合は、localized label 共有以外の責務を持たせないこと (持たせるならアプリ固有 sub に降ろす)。 |
...
| `StandardAppearanceMode` | `system / light / dark` の 3 case enum + `preferredColorScheme: ColorScheme?` 変換。`applyAppAppearance(_:)` の自然な入力源。consumer は `@AppStorage("...")` で raw value を永続化し、復元時に `StandardAppearanceMode(rawValue:) ?? .system` で受ける。**永続化キーや UserDefaults は consumer の関心** なので lib には載せない。 |
...
| API | 役割 |
|-----|------|
| `StandardAppComponentsLocalization.requiredKeys` | lib が同梱 View で使う必須ローカライズキーの一覧 (`General` / `Appearance` / `Language` / `Open at Login` / `Show in Menu Bar` / LanguageSection 関連 5 件)。 |
