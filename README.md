...
| `StandardAppLogging` | `import StandardAppLogging` | **UI 非依存** | line log は dual-sink (`os.Logger` 常時 + DEBUG stderr ミラー)、structured は category 規約付き `os.Logger`。UI を背負えない Infrastructure 等のレイヤーからも依存できるよう別 product にしている |
...
enum MyLogCategory: String, LogCategory {
    ...
    var defaultMessagePrivacy: LogPrivacy {   // line log 専用
        switch self {
...
// structured / OSLog-native (補間ごとの privacy)
appLog.osLogger(for: .network).error(
    "status=\(code, privacy: .public) token=\(token, privacy: .private)"
)
```
...
UI 非依存のロギング product。**ログの入口と subsystem / category 規約を `AppLog` に統一**する (callsite は `Logger(subsystem:category:)` を手打ちしない)。lib が全部を制御するのではなく、line log は `AppLog` が整形 / privacy / stderr ミラーを制御し、structured レーンは `AppLog` から category 規約付き `Logger` を払い出して OSLog-native な補間と privacy は `os.Logger` に委譲する。

| レーン | 入口 | privacy | sink |
|---|---|---|---|
| **line log** | `appLog.info(.app, "msg")` 等 | category 単位 / メッセージ全体 (AppLog 制御) | `os.Logger` 常時 + DEBUG stderr ミラー |
| **structured / OSLog-native** | `appLog.osLogger(for: .network)` → `Logger` | 補間ごとに `privacy:` 明示 (os.Logger に委譲) | `os.Logger` のみ |

どちらも subsystem は `AppLog`、category は `LogCategory` 由来に固定される。

導入時に知っておくべき要点 (詳細は [docs/logging.md](docs/logging.md)):

- **line log の privacy は category 単位の all-or-nothing** (`defaultMessagePrivacy`)。`.private` は release でメッセージ全体が `<private>` に畳まれ、`.public` は何も秘匿しない。category は安全網ではないので、**secret を含みうる値は category に関係なく callsite で sanitize してから渡す**。
- **補間ごとに privacy を分けたいときは structured レーン** (`appLog.osLogger(for:)`) を使い、返った `Logger` に直接書く。`os.Logger` のメソッドは補間リテラルを呼び出し箇所に要求するため、現行の Swift / OSLog API では薄い wrapper (`structured().error(OSLogMessage)`) を作れず、raw `Logger` を返す形になっている (詳細は [docs/logging.md](docs/logging.md))。
- **structured レーンは stderr ミラーしない** (field 単位 privacy を平文再構成しないため `os.Logger` のみ)。stderr でも見たい要約は line log を別途出す。
...
既定 (`defaultColorize`): release は常に OFF。DEBUG は env 自動判定で、**`NO_COLOR` / `CI` / `TERM=dumb` のいずれかがあれば OFF**、無ければ ON。env と無関係に強制したいときだけ `AppLog(subsystem: "...", colorize: false)` を明示する。
