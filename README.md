...
Personal SPM. macOS アプリの **「設定 (Settings) ウィンドウ」を中心とした共通化** を提供する社内 SPM。
複数の社内 macOS アプリ (ThumbnailThumb / vlc-multi-video-player / DualNote 等) で繰り返し書かれていた Settings まわりの定型コード ── タブ構成、ESC で閉じる挙動、外観切替、言語切替、`SMAppService` ラッパー、ローカライズ漏れ検証、`Settings × Form.formStyle(.grouped)` の罠対策 ── を 1 箇所に集約する。
