...
- **入口を統一する 2 レーン**: ログは常に `appLog` から出す。通常は line log、構造化は `appLog.osLogger(for:)`。callsite が `Logger(subsystem:category:)` を手打ちしないので、subsystem / category 規約が崩れない。
...
(実測 2026-06-21。`LogPrivacy` の「`OSLogMessage` は関数境界を越えられない」制約の一形態。) つまり補間ごとの privacy を保ったまま `Logger` 呼び出しを wrap するのは原理的に不可能で、structured レーンは「category 規約付きの raw `Logger` を返す」形しか取れない。`StructuredAppLog.error(_ message: OSLogMessage)` のような薄い型を再導入しないこと。
