...
| API | lib が提供するもの | consumer が実装するもの |
|---|---|---|
| `DoubleClickDetector` | 直近 click の時刻 / 位置と比較して「今の click が double か」を即時判定する helper。`NSEvent.doubleClickInterval` に floor (既定 0.3s) を噛ませ、"Double-click speed" 最速設定 (0.15s) で通常の double が不発になるのを救う。`checkDoubleClick(at:)` / `reset()` を持つ | click handler への配線 (single 動作 + `checkDoubleClick()` が true なら double 動作)。**row / cell ごとに `@State` で別 instance を持つこと** (detector を共有して `at:` 省略すると別 row への 2 連 click を double 誤判定する)。floor / 距離閾値を変える場合の `init` 引数選択 |
